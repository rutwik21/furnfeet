"use strict";
// export const PRODUCT_SUB_CATEGORIES = [
//   { label: 'Sofas' as const, value: 'sofa' as const},
//   { label: 'L-Shape Sofa' as const, value: 'l_shape_sofa' as const},
//   { label: '3 Seat Sofa' as const, value: '3_seat_sofa' as const},
//   { label: '2 Seat Sofa' as const, value: '2_seat_sofa' as const},
//   { label: '1 Seat Sofa' as const, value: '1_seat_sofa' as const},
//   { label: 'Recliners' as const, value: 'recliners' as const},
//   { label: '3 Seat Recliner' as const, value: '3_seat_recliner' as const},
//   { label: '2 Seat Recliner' as const, value: '2_seat_recliner' as const},
//   { label: '1 Seat Recliner' as const, value: '1_seat_recliner' as const},
//   { label: 'Sofa Cum Beds' as const, value: 'sofa_cum_bed' as const},
//   { label: 'Bean Bags' as const, value: 'bean_bag' as const},
//   { label: 'Spring mattresses' as const, value: 'spring_mattresses' as const},
//   { label: 'Pocket Spring Mattress' as const, value: 'pocket_spring_mattress' as const},
//   { label: 'Bonnell Spring Mattress' as const, value: 'bonnell_spring_mattress' as const},
//   { label: 'Memory Foam Mattress' as const, value: 'memory_foam_mattress' as const},
//   { label: 'Latex Foam Mattress' as const, value: 'latex_foam_mattress' as const},
//   { label: 'Bonded Foam Mattress' as const, value: 'bonded_foam_mattress' as const},
//   { label: 'Coir Mattress' as const, value: 'coir_mattress' as const},
//   { label: 'Vases' as const, value: 'vases' as const},
//   { label: 'Table Vases' as const, value: 'table_vases' as const},
//   { label: 'Floor Vases' as const, value: 'floor_vases' as const},
//   { label: 'Table Decor' as const, value: 'table_decor' as const},
//   { label: 'Decorative Boxes' as const, value: 'decorative_boxes' as const},
//   { label: 'Desk Organizers' as const, value: 'desk_organizers' as const},
//   { label: 'Magazine Racks' as const, value: 'magazine_racks' as const},
//   { label: 'Photo Frames' as const, value: 'photo_frames' as const},
//   { label: 'Single Photo Frames' as const, value: 'single' as const},
//   { label: 'Collage Photo Frames' as const, value: 'collage' as const},
//   { label: 'Clip Photo Photo Frames' as const, value: 'clip_photo' as const},
//   { label: 'Candles' as const, value: 'candles' as const},
//   { label: 'Decorative Candles' as const, value: 'decorative_candles' as const},
//   { label: 'Scented Candles' as const, value: 'scented_candles' as const},
//   { label: 'Tea Lights Candles' as const, value: 'tea_lights_candles' as const},
//   { label: 'Outdoor Decor' as const, value: 'outdoor_decor' as const},
//   { label: 'Artificial Grass' as const, value: 'artificial_grass' as const},
//   { label: 'Artificial Plants' as const, value: 'artificial_plants' as const},
//   { label: 'Artificial Flowers' as const, value: 'artificial_flowers' as const},
//   { label: 'Vertical Garden' as const, value: 'vertical_garden' as const},
//   // { label: 'Wall Decor' as const, value: 'wall_decor' as const},
//   // { label: 'Wall Art' as const, value: 'wall_art' as const},
//   // { label: 'Wall Hangings' as const, value: 'wall_hangings' as const},
//   // { label: 'Wallpaper for Wall' as const, value: 'wallpaper_for_wall' as const},
//   // { label: 'Wall Stickers' as const, value: 'wall_stickers' as const},
//   { label: 'Wall Decor' as const, value: 'wall_decor' as const},
//   { label: 'Wall Art' as const, value: 'wall_art' as const},
//   { label: 'Wall Hangings' as const, value: 'wall_hangings' as const},
//   { label: 'Wallpaper for Wall' as const, value: 'wallpaper_for_wall' as const},
//   { label: 'Wall Stickers' as const, value: 'wall_stickers' as const},
//   { label: 'Bed Sheets' as const, value: 'bed_sheets' as const},
//   { label: 'King Bed Sheets' as const, value: 'king_bed_sheets' as const},
//   { label: 'Queen Bed Sheets' as const, value: 'queen_bed_sheets' as const},
//   { label: 'Single Bed Sheets' as const, value: 'single_bed_sheets' as const},
//   { label: 'Blankets & Quilts' as const, value: 'blankets_and_quilts' as const},
//   { label: 'Quilts' as const, value: 'quilts' as const},
//   { label: 'Dohars' as const, value: 'dohars' as const},
//   { label: 'Blankets' as const, value: 'blankets' as const},
//   { label: 'Curtains' as const, value: 'curtains' as const},
//   { label: 'Door Curtains' as const, value: 'door_curtains' as const},
//   { label: 'Window Curtains' as const, value: 'window_curtains' as const},
//   { label: 'Blinds' as const, value: 'blinds' as const},
//   { label: 'Carpets' as const, value: 'carpets' as const},
//   { label: '3 ft x 5 ft Carpet' as const, value: '3_by_5' as const},
//   { label: '5 ft x 7 ft Carpet' as const, value: '5_by_7' as const},
//   { label: '6 ft x 9 ft Carpet' as const, value: '6_by_9' as const},
//   { label: '8 ft x 10 ft Carpet' as const, value: '8_by_10' as const},
//   { label: '9 ft x 12 ft Carpet' as const, value: '9_by_12' as const},
//   { label: 'Cushions & Covers' as const, value: 'cushions_and_covers' as const},
//   { label: 'Pillows' as const, value: 'pillow' as const},
//   { label: 'Pillow Covers' as const, value: 'pillow_covers' as const},
//   { label: 'Cushions' as const, value: 'cushions' as const},
//   { label: 'Cushions Cover' as const, value: 'cushion_cover' as const},
//   { label: 'Bolster' as const, value: 'bolster' as const},
//   { label: 'Furnishings' as const, value: 'furnishings' as const},
//   { label: 'Towels' as const, value: 'towels' as const},
//   { label: 'Comforters' as const, value: 'comforters' as const},
//   { label: 'Door Mats' as const, value: 'door_mats' as const}
// ];
Object.defineProperty(exports, "__esModule", { value: true });
exports.PRODUCT_CATEGORIES = void 0;
exports.PRODUCT_CATEGORIES = [
    {
        label: 'Sofas & Seating',
        value: 'sofa_and_seating',
        featured: [
            {
                name: 'Sofas',
                value: 'sofa',
                href: `/products?category=sofa_and_seating`,
                subCategories: [
                    {
                        name: 'L-Shape Sofa',
                        value: 'l_shape_sofa',
                        href: `/products?category=sofa_and_seating&subcategory=l_shape_sofa`,
                    },
                    {
                        name: '3 Seat Sofa',
                        value: '3_seat_sofa',
                        href: `/products?category=sofa_and_seating&subcategory=3_seat_sofa`,
                    },
                    {
                        name: '2 Seat Sofa',
                        value: '2_seat_sofa',
                        href: `/products?category=sofa_and_seating&subcategory=2_seat_sofa`,
                    },
                    {
                        name: '1 Seat Sofa',
                        value: '1_seat_sofa',
                        href: `/products?category=sofa_and_seating&subcategory=1_seat_sofa`,
                    },
                ]
            },
            {
                name: 'Recliners',
                value: 'recliners',
                href: '/products?category=sofa_and_seating&subcategory=recliners',
                subCategories: [
                    {
                        name: '3 Seat Recliner',
                        value: '3_seat_recliner',
                        href: `/products?category=sofa_and_seating&subcategory=3_seat_recliner`,
                    },
                    {
                        name: '2 Seat Recliner',
                        value: '2_seat_recliner',
                        href: `/products?category=sofa_and_seating&subcategory=2_seat_recliner`,
                    },
                    {
                        name: '1 Seat Recliner',
                        value: '1_seat_recliner',
                        href: `/products?category=sofa_and_seating&subcategory=1_seat_recliner`,
                    },
                ]
            },
            {
                name: 'Sofa Cum Beds',
                value: 'sofa_cum_bed',
                href: '/products?category=sofa_and_seating&subcategory=sofa_cum_bed',
                subCategories: []
            },
            {
                name: 'Bean Bags',
                value: 'bean_bag',
                href: `/products?category=sofa_and_seating&subcategory=bean_bag`,
                subCategories: []
            },
            {
                name: 'New Arrivals',
                value: '',
                href: '/products?category=sofa_and_seating&sort=desc',
                subCategories: []
            },
        ],
        categoryImgSrc: '/nav/sofa/sofa-img.png'
    },
    {
        label: 'Mattresses',
        value: 'mattresses',
        featured: [
            {
                name: 'Spring mattresses',
                value: 'spring_mattresses',
                href: `/products?category=mattresses&subcategory=spring_mattresses`,
                subCategories: [
                    {
                        name: 'Pocket Spring Mattress',
                        value: 'pocket_spring_mattress',
                        href: `/products?category=mattresses&subcategory=pocket_spring_mattress`,
                    },
                    {
                        name: 'Bonnell Spring Mattress',
                        value: 'bonnell_spring_mattress',
                        href: `/products?category=mattresses&subcategory=bonnell_spring_mattress`,
                    },
                ]
            },
            {
                name: 'Memory Foam Mattress',
                value: 'memory_foam_mattress',
                href: `/products?category=mattresses&subcategory=memory_foam_mattress`,
                subCategories: []
            },
            {
                name: 'Latex Foam Mattress',
                value: 'latex_foam_mattress',
                href: '/products?category=mattresses&subcategory=latex_foam_mattress',
                subCategories: []
            },
            {
                name: 'Bonded Foam Mattress',
                value: 'bonded_foam_mattress',
                href: '/products?category=mattresses&subcategory=bonded_foam_mattress',
                subCategories: []
            },
            {
                name: 'Coir Mattress',
                value: 'coir_mattress',
                href: '/products?category=mattresses&subcategory=coir_mattress',
                subCategories: []
            },
        ],
        categoryImgSrc: '/nav/mattress/mattress-img.png'
    },
    {
        label: 'Home Decor',
        value: 'home_decor',
        featured: [
            {
                name: 'Vases',
                value: 'vases',
                href: `/products?category=home_decor&subcategory=vases`,
                subCategories: [
                    {
                        name: 'Table Vases',
                        value: 'table_vases',
                        href: `/products?category=home_decor&subcategory=table_vases`,
                    },
                    {
                        name: 'Floor Vases',
                        value: 'floor_vases',
                        href: `/products?category=home_decor&subcategory=floor_vases`,
                    },
                ]
            },
            {
                name: 'Table Decor',
                value: 'table_decor',
                href: '/products?category=home_decor&subcategory=table_decor',
                subCategories: [
                    {
                        name: 'Decorative Boxes',
                        value: 'decorative_boxes',
                        href: `/products?category=home_decor&subcategory=decorative_boxes`,
                    },
                    {
                        name: 'Desk Organizers',
                        value: 'desk_organizers',
                        href: `/products?category=home_decor&subcategory=desk_organizers`,
                    },
                    {
                        name: 'Magazine Racks',
                        value: 'magazine_racks',
                        href: `/products?category=home_decor&subcategory=magazine_racks`,
                    },
                ]
            },
            {
                name: 'Photo Frames',
                value: 'photo_frames',
                href: '/products?category=home_decor&subcategory=photo_frames',
                subCategories: [
                    {
                        name: 'Single',
                        value: 'single',
                        href: `/products?category=home_decor&subcategory=single`,
                    },
                    {
                        name: 'Collage',
                        value: 'collage',
                        href: `/products?category=home_decor&subcategory=collage`,
                    },
                    {
                        name: 'Clip Photo',
                        value: 'clip_photo',
                        href: `/products?category=home_decor&subcategory=clip_photo`,
                    },
                ]
            },
            {
                name: 'Candles',
                value: 'candles',
                href: `/products?category=home_decor&subcategory=candles`,
                subCategories: [
                    {
                        name: 'Decorative Candles',
                        value: 'decorative_candles',
                        href: `/products?category=home_decor&subcategory=decorative_candles`,
                    },
                    {
                        name: 'Scented Candles',
                        value: 'scented_candles',
                        href: `/products?category=home_decor&subcategory=scented_candles`,
                    },
                    {
                        name: 'Tea Lights Candles',
                        value: 'tea_lights_candles',
                        href: `/products?category=home_decor&subcategory=tea_lights_candles`,
                    },
                ]
            },
            {
                name: 'Outdoor Decor',
                value: 'outdoor_decor',
                href: '/products?category=home_decor&subcategory=outdoor_decor',
                subCategories: [
                    {
                        name: 'Artificial Grass',
                        value: 'artificial_grass',
                        href: `/products?category=home_decor&subcategory=artificial_grass`,
                    },
                    {
                        name: 'Artificial Plants',
                        value: 'artificial_plants',
                        href: `/products?category=home_decor&subcategory=artificial_plants`,
                    },
                    {
                        name: 'Artificial Flowers',
                        value: 'artificial_flowers',
                        href: `/products?category=home_decor&subcategory=artificial_flowers`,
                    },
                    {
                        name: 'Vertical Garden',
                        value: 'vertical_garden',
                        href: `/products?category=home_decor&subcategory=vertical_garden`,
                    },
                ]
            },
            {
                name: 'Wall Decor',
                value: 'wall_decor',
                href: '/products?category=home_decor&subcategory=wall_decor',
                subCategories: [
                    {
                        name: 'Wall Art',
                        value: 'wall_art',
                        href: `/products?category=home_decor&subcategory=wall_art`,
                    },
                    {
                        name: 'Wall Hangings',
                        value: 'wall_hangings',
                        href: `/products?category=home_decor&subcategory=wall_hangings`,
                    },
                    {
                        name: 'Wallpaper for Wall',
                        value: 'wallpaper_for_wall',
                        href: `/products?category=home_decor&subcategory=wallpaper_for_wall`,
                    },
                    {
                        name: 'Wall Stickers',
                        value: 'wall_stickers',
                        href: `/products?category=home_decor&subcategory=wall_stickers`,
                    },
                ]
            },
            {
                name: 'Wall Decor',
                value: 'wall_decor',
                href: '/products?category=home_decor&subcategory=wall_decor',
                subCategories: [
                    {
                        name: 'Wall Art',
                        value: 'wall_art',
                        href: `/products?category=home_decor&subcategory=wall_art`,
                    },
                    {
                        name: 'Wall Hangings',
                        value: 'wall_hangings',
                        href: `/products?category=home_decor&subcategory=wall_hangings`,
                    },
                    {
                        name: 'Wallpaper for Wall',
                        value: 'wallpaper_for_wall',
                        href: `/products?category=home_decor&subcategory=wallpaper_for_wall`,
                    },
                    {
                        name: 'Wall Stickers',
                        value: 'wall_stickers',
                        href: `/products?category=home_decor&subcategory=wall_stickers`,
                    },
                ]
            },
        ],
        categoryImgSrc: '/nav/homeDecor/home-decor.png'
    },
    {
        label: 'Furnishings',
        value: 'furnishings',
        featured: [
            {
                name: 'Bed Sheets',
                value: 'bed_sheets',
                href: `/products?category=furnishings&subcategory=bed_sheets`,
                subCategories: [
                    {
                        name: 'King Bed Sheets',
                        value: 'king_bed_sheets',
                        href: `/products?category=furnishings&subcategory=king_bed_sheets`,
                    },
                    {
                        name: 'Queen Bed Sheets',
                        value: 'queen_bed_sheets',
                        href: `/products?category=furnishings&subcategory=queen_bed_sheets`,
                    },
                    {
                        name: 'Single Bed Sheets',
                        value: 'single_bed_sheets',
                        href: `/products?category=furnishings&subcategory=single_bed_sheets`,
                    },
                ]
            },
            {
                name: 'Blankets & Quilts',
                value: 'blankets_and_quilts',
                href: '/products?category=furnishings&subcategory=blankets_and_quilts',
                subCategories: [
                    {
                        name: 'Quilts',
                        value: 'quilts',
                        href: `/products?category=furnishings&subcategory=quilts`,
                    },
                    {
                        name: 'Dohars',
                        value: 'dohars',
                        href: `/products?category=furnishings&subcategory=dohars`,
                    },
                    {
                        name: 'Blankets',
                        value: 'blankets',
                        href: `/products?category=furnishings&subcategory=blankets`,
                    },
                ]
            },
            {
                name: 'Curtains',
                value: 'curtains',
                href: '/products?category=furnishings&subcategory=curtains',
                subCategories: [
                    {
                        name: 'Door Curtains',
                        value: 'door_curtains',
                        href: `/products?category=furnishings&subcategory=door_curtains`,
                    },
                    {
                        name: 'Window Curtains',
                        value: 'window_curtains',
                        href: `/products?category=furnishings&subcategory=window_curtains`,
                    },
                    {
                        name: 'Blinds',
                        value: 'blinds',
                        href: `/products?category=furnishings&subcategory=blinds`,
                    },
                ]
            },
            {
                name: 'Carpets',
                value: 'carpets',
                href: `/products?category=furnishings&subcategory=carpets`,
                subCategories: [
                    {
                        name: '3 ft x 5 ft',
                        value: '3_by_5',
                        href: `/products?category=furnishings&subcategory=3_by_5`,
                    },
                    {
                        name: '5 ft x 7 ft',
                        value: '5_by_7',
                        href: `/products?category=furnishings&subcategory=5_by_7`,
                    },
                    {
                        name: '6 ft x 9 ft',
                        value: '6_by_9',
                        href: `/products?category=furnishings&subcategory=6_by_9`,
                    },
                    {
                        name: '8 ft x 10 ft',
                        value: '8_by_10',
                        href: `/products?category=furnishings&subcategory=8_by_10`,
                    },
                    {
                        name: '9 ft x 12 ft',
                        value: '9_by_12',
                        href: `/products?category=furnishings&subcategory=9_by_12`,
                    },
                ]
            },
            {
                name: 'Cushions & Covers',
                value: 'cushions_and_covers',
                href: '/products?category=furnishings&subcategory=cushions_and_covers',
                subCategories: [
                    {
                        name: 'Pillows',
                        value: 'pillow',
                        href: `/products?category=furnishings&subcategory=pillow`,
                    },
                    {
                        name: 'Pillow Covers',
                        value: 'pillow_covers',
                        href: `/products?category=furnishings&subcategory=pillow_covers`,
                    },
                    {
                        name: 'Cushions',
                        value: 'cushions',
                        href: `/products?category=furnishings&subcategory=cushions`,
                    },
                    {
                        name: 'Cushions Cover',
                        value: 'cushion_cover',
                        href: `/products?category=furnishings&subcategory=cushion_cover`,
                    },
                    {
                        name: 'Bolster',
                        value: 'bolster',
                        href: `/products?category=furnishings&subcategory=bolster`,
                    },
                ]
            },
            {
                name: 'Furnishings',
                value: 'furnishings',
                href: '/products?category=furnishings',
                subCategories: [
                    {
                        name: 'Towels',
                        value: 'towels',
                        href: `/products?category=furnishings&subcategory=towels`,
                    },
                    {
                        name: 'Comforters',
                        value: 'comforters',
                        href: `/products?category=furnishings&subcategory=comforters`,
                    },
                    {
                        name: 'Door Mats',
                        value: 'door_mats',
                        href: `/products?category=furnishings&subcategory=door_mats`,
                    },
                ]
            },
        ],
        categoryImgSrc: '/nav/furnishing/furnishing.png'
    },
    {
        label: 'Raw Materials',
        value: 'raw_materials',
        featured: [
            {
                name: 'Fabric',
                value: 'fabric',
                href: `/products?category=raw_materials&subcategory=fabric`,
                subCategories: [
                    {
                        name: 'Sofa Fabric',
                        value: 'sofa_fabric',
                        href: `/products?category=raw_materials&subcategory=sofa_fabric`,
                    },
                    {
                        name: 'Curtain Fabric',
                        value: 'curtain_fabric',
                        href: `/products?category=raw_materials&subcategory=curtain_fabric`,
                    },
                ]
            },
            {
                name: 'Foam',
                value: 'foam',
                href: `/products?category=raw_materials&subcategory=foam`,
                subCategories: [
                    {
                        name: '50 Density',
                        value: '50Density',
                        href: `/products?category=raw_materials&subcategory=50Density`,
                    },
                    {
                        name: '40 Density',
                        value: '40Density',
                        href: `/products?category=raw_materials&subcategory=40Density`,
                    },
                    {
                        name: '30 Density',
                        value: '30Density',
                        href: `/products?category=raw_materials&subcategory=30Density`,
                    },
                ]
            },
            // {
            //   name: 'Ply',
            //   value: 'ply' as const,
            //   href: `/products?category=raw_materials&subcategory=ply`,
            //   subCategories: [
            //     {
            //       name: '18mm',
            //       value: '18mm' as const,
            //       href: `/products?category=raw_materials&subcategory=18mm`,
            //     },
            //     {
            //       name: '12mm',
            //       value: '12mm' as const,
            //       href: `/products?category=raw_materials&subcategory=12mm`,
            //     },
            //   ]
            // },
        ],
        categoryImgSrc: '/nav/furnishing/furnishing.png'
    },
    // {
    //   label: 'Lamps & Lights',
    //   value: 'lamps_and_lights' as const,
    //   featured: [
    //     {
    //       name: 'Favorite Icon Picks',
    //       value: 'lamps_and_lights' as const,
    //       href: `/products?category=lamps_and_lights&subcategory=3_seat_recliner`,
    //       subCategories: [
    //         {
    //           name: '3 Seat Recliner',
    //           value: 'lamps_and_lights' as const,
    //           href: `/products?category=lamps_and_lights&subcategory=3_seat_recliner`,
    //         },
    //         {
    //           name: '2 Seat Recliner',
    //           value: 'lamps_and_lights' as const,
    //           href: `/products?category=lamps_and_lights&subcategory=2_seat_recliner`,
    //         },
    //         {
    //           name: '1 Seat Recliner',
    //           value: 'lamps_and_lights' as const,
    //           href: `/products?category=lamps_and_lights&subcategory=1_seat_recliner`,
    //         },
    //       ]
    //     },
    //     {
    //       name: 'New Arrivals',
    //       value: 'lamps_and_lights' as const,
    //       href: '/products?category=lamps_and_lights&subcategory=3_seat_recliner&sort=desc',
    //       subCategories: [
    //         {
    //           name: '3 Seat Recliner',
    //           value: 'lamps_and_lights' as const,
    //           href: `/products?category=lamps_and_lights&subcategory=3_seat_recliner`,
    //         },
    //         {
    //           name: '2 Seat Recliner',
    //           value: 'lamps_and_lights' as const,
    //           href: `/products?category=lamps_and_lights&subcategory=2_seat_recliner`,
    //         },
    //         {
    //           name: '1 Seat Recliner',
    //           value: 'lamps_and_lights' as const,
    //           href: `/products?category=lamps_and_lights&subcategory=1_seat_recliner`,
    //         },
    //       ]
    //     },
    //     {
    //       name: 'Bestselling Icons',
    //       value: 'lamps_and_lights' as const,
    //       href: '/products?category=lamps_and_lights&subcategory=3_seat_recliner',
    //       subCategories: [
    //         {
    //           name: '3 Seat Recliner',
    //           value: 'lamps_and_lights' as const,
    //           href: `/products?category=lamps_and_lights&subcategory=3_seat_recliner`,
    //         },
    //         {
    //           name: '2 Seat Recliner',
    //           value: 'lamps_and_lights' as const,
    //           href: `/products?category=lamps_and_lights&subcategory=2_seat_recliner`,
    //         },
    //         {
    //           name: '1 Seat Recliner',
    //           value: 'lamps_and_lights' as const,
    //           href: `/products?category=lamps_and_lights&subcategory=1_seat_recliner`,
    //         },
    //       ]
    //     },
    //     {
    //       name: 'Favorite Icon Picks',
    //       value: 'lamps_and_lights' as const,
    //       href: `/products?category=lamps_and_lights&subcategory=3_seat_recliner`,
    //       subCategories: [
    //         {
    //           name: '3 Seat Recliner',
    //           value: 'lamps_and_lights' as const,
    //           href: `/products?category=lamps_and_lights&subcategory=3_seat_recliner`,
    //         },
    //         {
    //           name: '2 Seat Recliner',
    //           value: 'lamps_and_lights' as const,
    //           href: `/products?category=lamps_and_lights&subcategory=2_seat_recliner`,
    //         },
    //         {
    //           name: '1 Seat Recliner',
    //           value: 'lamps_and_lights' as const,
    //           href: `/products?category=lamps_and_lights&subcategory=1_seat_recliner`,
    //         },
    //       ]
    //     },
    //     {
    //       name: 'New Arrivals',
    //       value: 'lamps_and_lights' as const,
    //       href: '/products?category=lamps_and_lights&subcategory=3_seat_recliner&sort=desc',
    //       subCategories: [
    //         {
    //           name: '3 Seat Recliner',
    //           value: 'lamps_and_lights' as const,
    //           href: `/products?category=lamps_and_lights&subcategory=3_seat_recliner`,
    //         },
    //         {
    //           name: '2 Seat Recliner',
    //           value: 'lamps_and_lights' as const,
    //           href: `/products?category=lamps_and_lights&subcategory=2_seat_recliner`,
    //         },
    //         {
    //           name: '1 Seat Recliner',
    //           value: 'lamps_and_lights' as const,
    //           href: `/products?category=lamps_and_lights&subcategory=1_seat_recliner`,
    //         },
    //       ]
    //     },
    //     {
    //       name: 'Bestselling Icons',
    //       value: 'lamps_and_lights' as const,
    //       href: '/products?category=lamps_and_lights&subcategory=3_seat_recliner',
    //       subCategories: [
    //         {
    //           name: '3 Seat Recliner',
    //           value: 'lamps_and_lights' as const,
    //           href: `/products?category=lamps_and_lights&subcategory=3_seat_recliner`,
    //         },
    //         {
    //           name: '2 Seat Recliner',
    //           value: 'lamps_and_lights' as const,
    //           href: `/products?category=lamps_and_lights&subcategory=2_seat_recliner`,
    //         },
    //         {
    //           name: '1 Seat Recliner',
    //           value: 'lamps_and_lights' as const,
    //           href: `/products?category=lamps_and_lights&subcategory=1_seat_recliner`,
    //         },
    //       ]
    //     },
    //   ],
    //   categoryImgSrc: '/nav/lights/lights.png'
    // },
];
